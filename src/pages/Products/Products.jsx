    ...
    return (
        <div className="max-w-sm px-6 md:max-w-3xl md:px-8 lg:max-w-7xl mx-auto lg:mt-12 mb-12">
            ...
            <div className={`grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2 gap-6`}>
                {
                    Products.map(p => <div key={p._id} className="animate__animated animate__zoomIn shadow-lg h-[400px] md:h-[540px] relative">
                        <div className="">
                            <img src={p.productImage} className="" />
                            <div>
                                <h2 className="md:text-[16px] text-[12px] font-bold text-center px-2">
                                    {p.productName.length > 25
                                        ? `${p.productName.slice(0, 25)}...`
                                        : p.productName
                                    }</h2>

                                    <p className='text-center font-mono font-semibold py-1 absolute bottom-14 md:left-16 left-7'>Tk. {p.price}</p>
                            </div>
                        </div>
